...
export default function Notes() {
  ...
  const [editingNoteId, setEditingNoteId] = useState(undefined); // undefined = modal closed, null = create new, string = edit
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="flex gap-1">
                    <button
                      ...
                      className={`p-1 ${note.pinned ? 'text-indigo-500' : 'text-gray-400'}`}
                    >
                      <Pin size={16} />
                    </button>
                    <button
                ...
                <div className="flex gap-2 mt-3 flex-wrap">
                  {note.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-xs bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 px-2 py-0.5 rounded-full"
                    >
                      {tag}
                    </span>
                  ))}
